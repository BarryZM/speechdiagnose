...
def handler(event, context):
    ...
    for index_key in input.keys():
        if input[index_key] and (index_key not in handler.input_last.keys() ) :
            ...
            if index_num  <= 0: # index_sub = object_index
                index_main = -index_num
            elif index_num > 0: # index_sub = object_index*10
    ...
    for key in handler.diagnoses.keys():
        num_match = 0
        for i in indexes:
            if handler.diagnoses[key][i] == descps[i]:
                num_match += 1
            else:
                break
        if num_match == indexes_len:
